...
def deductive_forward(cls,
    ...
    #cls_embeddings = outputs.last_hidden_state[:, 0, :].unsqueeze(1)
        ...
        # Step 1: Extracting Last Sentence Embeddings
    last_hidden_state = outputs.last_hidden_state
    # Directly compute the last sentence embeddings
    last_sentence_embeddings = []
    for batch_item in range(last_hidden_state.size(0)):  # Iterate over batch
        sentence_start = 0
        last_sentence_end = 0
        for i, token_id in enumerate(input_ids[batch_item]):
            if token_id == 479 or token_id == 2:  # Adjust token IDs based on your tokenizer
                last_sentence_end = i
        # Calculate the last sentence embedding
        if last_sentence_end > sentence_start:
            last_sentence_embedding = torch.mean(last_hidden_state[batch_item, sentence_start:last_sentence_end], dim=0)
        else:
            # Fallback to using the [CLS] token embedding if no sentence detected
            last_sentence_embedding = last_hidden_state[batch_item, 0]
        last_sentence_embeddings.append(last_sentence_embedding)

    # Convert list to tensor
    last_sentence_embeddings = torch.stack(last_sentence_embeddings)

    # Score the Last Sentence Embedding
    last_sentence_scores = cls.emb_scorer(last_sentence_embeddings)  # Shape: [batch_size, 1]
    ...
    for i in range(max_height):
        ...
        if i == 0:
            ...
            # 
            # Assuming the size along the second dimension should match `mi_logits`
            target_size = mi_logits.size(1)  # Assuming mi_logits has the correct target size
            last_sentence_scores_expanded = last_sentence_scores.unsqueeze(1).unsqueeze(2).expand(-1, target_size, -1, -1)



            mi_combined_logits = mi_logits + mi_stopper_logits + expanded_var_scores + last_sentence_scores_expanded
