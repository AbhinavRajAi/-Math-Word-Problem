...
def train(config: Config, train_dataloader: DataLoader, num_epochs: int,
    ...
    for epoch in range(num_epochs):
        ...
        if valid_dataloader is not None:
            equ_acc, val_acc_performance, Prediction, Label, gold_valu, pred_value, is_valu_corr = evaluate(valid_dataloader, model, dev, uni_labels=config.uni_labels, fp16=bool(config.fp16), constant_values=constant_values)
            test_equ_acc, test_val_acc = -1, -1

            if test_dataloader is not None:
                logger.info("The DATALOADER is working")
                test_equ_acc, test_val_acc, Predictions, Labels, gold_value, predi_value, is_valu_corre = evaluate(test_dataloader, model, dev, uni_labels=config.uni_labels, fp16=bool(config.fp16), constant_values=constant_values,
    ...
    logger.info(f"[Model Info] Predicted_Equation: {Prediction}")
    logger.info(f"[Model Info] Label_Equation: {Label}")
    logger.info(f"[Model Info] Predicted_Value: {pred_value}")
    logger.info(f"[Model Info] Label_Value: {gold_valu}")

    logger.info(f"[Model Info] Correct_or_not: {is_valu_corr}")
...
def evaluate(valid_dataloader: DataLoader, model: nn.Module, dev: torch.device, fp16:bool, constant_values: List, uni_labels:List,
    ...
    for inst_predictions, inst_labels, inst in zip(predictions, labels, insts):
        ...
        Label.append(gold_ground_equation)
        Prediction.append(pred_ground_equation)
        is_valu_corr.append(is_value_corr)
        pred_value.append(predict_value)
        gold_valu.append(gold_value)
    ...
    return acc, val_acc, Prediction, Label, gold_valu, pred_value, is_valu_corr
